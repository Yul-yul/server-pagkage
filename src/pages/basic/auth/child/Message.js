...
//服务配置
import serverList from '../../../json/serverList'
//数据库配置
...
//脚本名称
const batName = 'auth.bat'
//日志名称
const logName = 'auth.txt'
...
class Message extends Component {
  ...
  generate = () => {
    ...
    //定义配置
    let buffor = ""
    buffor = buffor + ' -Deureka.client.service-url.defaultZone=http://' + eureka.IP + ':' + eureka.port + '/eureka/ '
    //添加数据库信息
  ...
  onShowBat = () => {
    ...
    if (config.port != undefined) {
      buffor = buffor + " -Dserver.prot=" + config.port + " \\ \n"
    }
    spring.forEach((item, index) => {
      buffor = buffor + "-D" + item.key + "=" + item.value + " \\ \n"
    })
